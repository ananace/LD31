...
namespace
{
    ...
    sf::View* getDefaultView(sf::RenderWindow* target)
    {
        return const_cast<sf::View*>(&target->getDefaultView());
    }
    sf::View* getView(sf::RenderWindow* target)
    {
        return const_cast<sf::View*>(&target->getView());
    }
    void setView(sf::View* view, sf::RenderWindow* target)
    {
        target->setView(*view);
    }
#ifdef AS_SUPPORT_VALRET
    Math::Vector2 getSize(sf::RenderWindow* target)
    {
        return target->getSize();
    }
    Math::Rect getViewport(sf::View* view, sf::RenderWindow* target)
    {
        return target->getViewport(*view);
    }
    Math::Vector2 mapCoordsToPixel(const Math::Vector2& vec, sf::RenderWindow* target)
    {
        return target->mapCoordsToPixel(vec);
    }
    Math::Vector2 mapCoordsToPixel_view(const Math::Vector2& vec, sf::View* view, sf::RenderWindow* target)
    {
        return target->mapCoordsToPixel(vec, *view);
    }
    Math::Vector2 mapPixelToCoords(const Math::Vector2& vec, sf::RenderWindow* target)
    {
        return target->mapPixelToCoords(vec);
    }
    Math::Vector2 mapPixelToCoords_view(const Math::Vector2& vec, sf::View* view, sf::RenderWindow* target)
    {
    ...
    {
        sf::RenderWindow* target = reinterpret_cast<sf::RenderWindow*>(gen->GetObject());
    ...
    bool Reg()
    {
        Script::ScriptExtensions::AddExtension([](asIScriptEngine* eng) {
            ...
            r = eng->RegisterObjectMethod("Renderer", "void Clear(Color&in = Colors::Black)", asMETHOD(sf::RenderWindow, clear), asCALL_THISCALL); assert(r >= 0);
